...
void Manager::drawScaledSprite( std::list<ScaledSprite*>::const_iterator &ptr, float desiredScale) const {
	
  while(ptr != fruits.end() && (*ptr)->getScale() == desiredScale) {
    (*ptr)->draw();
    ptr++;
  }
...
void Manager::draw() const {
  ...
  std::list<ScaledSprite*>::const_iterator ptr = fruits.begin();
  night_sky.draw();
  drawScaledSprite(ptr,0.5);
  front_building.draw();
  drawScaledSprite(ptr,1.0);
  platform.draw();
  drawScaledSprite(ptr,1.5);
...
void Manager::update() {  
  ...
  platform.update();
//  health.update(ticks);
    std::list<ScaledSprite*>::iterator p = fruits.begin();
  while(p != fruits.end()){
    (*p)->update(ticks);
    bool isCollided = (*p)->collidedWith(sprites[0]);
   if(isCollided){
	 sprites[0]->explode();
	 (*p)->explode();
 }
