...
ExplodingSprite::ExplodingSprite(const Sprite& s) :
  ...
  surfaces(),
  frames(),
  expSpeedX(Gamedata::getInstance().getXmlInt(s.getName()+"/speedX")),
  expSpeedY(Gamedata::getInstance().getXmlInt(s.getName()+"/speedY")) {
  makeChunks(
...
void ExplodingSprite::makeChunks(unsigned int n) { 
  ...
  int speedy = expSpeedY; //static_cast<int>(velocityY()); // Make sure it's an int.
